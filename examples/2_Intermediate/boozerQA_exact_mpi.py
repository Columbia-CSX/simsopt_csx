...
from simsopt.geo import curves_to_vtk, MajorRadius, CurveLength, CurveCurveDistance, NonQuasiSymmetricRatio, Iotas,\
        BoozerResidualExact, LpCurveCurvature, MeanSquaredCurvature, ArclengthVariation, SurfaceXYZTensorFourier, Volume, BoozerSurface
...
proc0_print("Running 2_Intermediate/boozerQA_exact_mpi.py")
proc0_print("=========================================")

base_curves, base_currents, coils, curves, surfaces, boozer_surfaces, ress = load(IN_DIR + "ncsx_init.json")


# Transform to boozer exact
new_boozer_surfaces = []
for bsurf in boozer_surfaces:
    surf = bsurf.surface
    new_surf = SurfaceXYZTensorFourier(
        mpol=surf.mpol, ntor=surf.ntor,
        nfp=surf.nfp, stellsym=surf.stellsym,
        quadpoints_phi=np.linspace(0,1.0/surf.nfp,2*surf.ntor+1,endpoint=False),
        quadpoints_theta=np.linspace(0,1.0,2*surf.mpol+1,endpoint=False),
        dofs=surf.dofs
    )
    ...
    boozer_surface = BoozerSurface(
        bsurf.biotsavart, new_surf, vol, vol_target, None, options={'verbose':True}
    )

    new_boozer_surfaces.append(boozer_surface)

boozer_surfaces = new_boozer_surfaces
...
brs = [BoozerResidualExact(boozer_surface, BiotSavart(coils)) for boozer_surface in boozer_surfaces]
